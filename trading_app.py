...
def run_simulation(df, params):
    """
    Runs a single simulation pass with the given parameters.
                        ...
                        for j in range(i + 1, len(df)):
                            ...
                            if not trailing_active:
                                if day_high >= target_activation_price:
                                    ...
                                    peak_price = day_high
                                    # Immediate stop check on activation day?
...
with st.sidebar:
    ...
    interest_rate_pct = st.number_input("Cash Interest (%)", 4.5, step=0.1)
